...
def test_finance(driver, payload):
    ...
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == ("CPUV11 / IL Affinity"):
            option.click()
            time.sleep(5)
    elem = driver.find_element_by_id("ctl00_MainContent_btnPricingCode").click()
    time.sleep(3)
    elem = driver.find_element_by_id("ctl00_MainContent_ddlInitialOffer")
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == ("02A83B / Commercial"):
            option.click()
            time.sleep(5)

    elem = driver.find_element_by_id("ctl00_MainContent_ddlState")
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == ("Illinois"):
            option.click()
            time.sleep(3)
    # elem = driver.find_element_by_id("ctl00$MainContent$ddlTDSPType")
    # for option in elem.find_elements_by_tag_name('option'):
    #     if option.text.strip() == ("Residential"):
    #         option.click()
    #         time.sleep(3)       
    elem = driver.find_element_by_id("ctl00_MainContent_ddlUtils")
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == ("Commonwealth Edison"):
            option.click()
            time.sleep(3) 
    elem = driver.find_element_by_id("ctl00_MainContent_ddlFixedSupplyCharge")
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == ("0.07900"):
            option.click()
            time.sleep(3) 
