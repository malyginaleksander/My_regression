...
def test_awards(driver, payload):
    print(payload.tc, ' - Segments Paging')

    driver.get("http://epnet2.pt.nrgpl.us/Pricing#segments")
    driver.implicitly_wait(10)

    elem = driver.find_element_by_id("next_priceSegmentsPager").click()
    assert "View 101 - 200" in driver.page_source
    print ("next");
    
    elem = driver.find_element_by_id("prev_priceSegmentsPager").click()
    ...
    elem = driver.find_element_by_id("first_priceSegmentsPager").click()
    assert "View 1 - 100" in driver.page_source
    print ("first");
