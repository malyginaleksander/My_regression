...
#If rdi running becomes compatible with a qa CCC RDI job website environment in the future, change myEnvironment to be a parameter in the call and not hardcoded.
def run_RDI_File(rdiFileAbsolutePath):
    #Instructions from https://energyplus.atlassian.net/wiki/spaces/TEST/pages/463306909/Test+Cases+For+PORTAL-493
    myEnv = "pt"
    cnopts = pysftp.CnOpts()
    cnopts.hostkeys = None
    print("\nConnecting to pysftp sftp.qa.energypluscompany.com server\n")
    srv = pysftp.Connection(host="sftp.qa.energypluscompany.com", username="nrpcorrqa", password="eNQ2UAEE9rUAPg",
                            port=22, cnopts=cnopts)
    print("\nPutting my RDI file on sftp.qa.energypluscompany.com server through pysftp.\n")
    dirString = '/home/nrg'
    ...
    #webpageString = "https://ccc.%s.nrgpl.us" % myEnv
    #driver.get(webpageString)
    #jobsLink = driver.find_element_by_xpath("/html/body/nav/div/div[2]/ul[1]/li[4]/a")
    #jobsLink.click()
    
    #/ html / body / nav / div / div[2] / ul[1] / li[4] / a
    #CSS Selector:
    # fileTransformer > a:nth-child(1)
    #CSS Path:
    #html
    #body
    #nav.navbar.navbar - default
    #div.container - fluid
    #div  # bs-example-navbar-collapse-1.collapse.navbar-collapse ul.nav.navbar-nav li#fileTransformer a
    
    #webpageString = "https://ccc.%s.nrgpl.us/admin/file_transformer/" % myEnv
    #driver.get(webpageString)

    #webpageString = "https://ccc.%s.nrgpl.us/admin/file_transformer/jobrun/" % myEnv
    #driver.get(webpageString)
    
    #addJobRunButton = driver.find_element_by_xpath("/html/body/div/div[3]/div/ul/li[1]/a")
    #addJobRunButton.click()
    
    webpageString = "https://ccc.%s.nrgpl.us/admin/file_transformer/jobrun/add/" % myEnv
    driver.get(webpageString)
    
    jobRunMenuSubElement = driver.find_element_by_xpath("/html/body/div/div[3]/div/form/div/fieldset/div[1]/div/div/select/option[51]")
    jobRunMenuSubElement.click()

    jobRunSaveButton = driver.find_element_by_xpath("/html/body/div/div[3]/div/form/div/div[2]/input[1]")
    jobRunSaveButton.click()

    #< input type = "submit" value = "Save" class ="default" name="_save" >
    #/html/body/div/div[3]/div/form/div/div[2]/input[1]
    
    
    
    
    #Maybe we should delete all block 45 RDI jobs except one, and forbid adding more. 
    
    #Might want to select the "RDI With Block Size 45" before "UMS Mail Date"
    #But for now we'll just select the last "RDI With Block Size 45" in the list.
    #And this will be SUPER easy to break.  Might want to check the text on the XPath element just to make sure we have the right element.
    #Select the menu by id: "id_job"
    #option value of the last rdi job = 78.  Yes, it's different than what's found in the xpath option, and I don't know why.
    #xpath of the last rdi job...
    #/ html / body / div / div[3] / div / form / div / fieldset / div[1] / div / div / select / option[51]
    #Here's the JAVA for what I'm trying to do... with a different XPath in the example.
    #String text = driver.findElement(webdriver.By.xpath("//div/span")).getText();
    #Does a menu automatically expand if you tell Selenium to click an element in it?
    #CSS selector
    # id_job > option:nth-child(51)
    #CSS Path
    #html
    #body.app - file_transformer.model - jobrun.change - form
    #div  # container div#content.colM div#content-main form#jobrun_form div fieldset.module.aligned div.form-row.field-job div div.related-widget-wrapper select#id_job option
