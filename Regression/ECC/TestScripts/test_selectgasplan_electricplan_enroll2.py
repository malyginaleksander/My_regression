...
worksheet = workbook.sheet_by_name('Gas_Electric_plan_enroll')

gas_electric_enrollment = []
...
for current_row in range(1, worksheet.nrows):
    ...
    gas_electric_enrollment.append(Payload(**value_dict))
...
@pytest.mark.parametrize("payload", gas_electric_enrollment, ids=[p.tc for p in gas_electric_enrollment])

def test_dual_enrollment(driver, payload):
     print( payload.tc, 'NRG_regression WEB Electric Enrollment - State = ', payload.state )

     try:
         _gas_electric_enrollment(driver, payload)
     except Exception as ae:
        import uuid
        filename = "./failed/test_gas_electric_enrollment_fail_{}_{}.png".format(payload.tc,uuid.uuid4())
...
def _gas_electric_enrollment(driver, payload):
        ...
        NrgEn.click_naturalgasplan()
        time.sleep(2)
        NrgEn.select_gas_utility_planspage(payload)
        time.sleep(2)
        NrgEn.select_gas_plan_planspage_new2(payload)
        time.sleep(2)
        NrgEn.select_electric_utility_confirmplanpage(payload)
        time.sleep(3)
        NrgEn.select_electric_plan_confirmplanpage(payload)
        NrgEn.click_ConfirmYourPlanLabel()
