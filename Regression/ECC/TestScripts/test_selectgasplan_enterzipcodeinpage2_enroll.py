...
worksheet = workbook.sheet_by_name('Gas_Plan_Enroll')
...
for current_row in range(1, worksheet.nrows):
    ...
    gas_enrollment.append(Payload(**value_dict))
...
@pytest.mark.parametrize("payload", gas_enrollment, ids=[p.tc for p in gas_enrollment])

def test_gas_enrollment(driver, payload):
     print( payload.tc, 'NRG_regression WEB Electric Enrollment - State = ', payload.state )

     try:
         _gas_enrollment(driver, payload)
     except Exception as ae:
        import uuid
        filename = "./failed/test_gas_enrollment_fail_{}_{}.png".format(payload.tc,uuid.uuid4())
...
def _gas_enrollment(driver, payload):
