...
worksheet = workbook.sheet_by_name('Error_resetpw')

resetpwpage_errormsgs = []
...
for current_row in range(1, worksheet.nrows):
    ...
    resetpwpage_errormsgs.append(Payload(**value_dict))
...
@pytest.mark.parametrize("payload", resetpwpage_errormsgs, ids=[p.tc for p in resetpwpage_errormsgs])
...
def _test_resetPassword(driver, payload):
        ...
        login = LoginPage( driver )
        login.errormessages_resetpassword(payload)
        time.sleep(3)
