...
for row in input_file_sap:
        for emailmarketing in email_mark:
                ...
                if sap_StateSlug.upper() == 'CT'.upper():
                    state_ = 'Connecticut'
                if sap_StateSlug.upper() == 'DE'.upper():
                    state_ = 'Delaware'
                if sap_StateSlug.upper() == 'IL'.upper():
                    state_ = 'Illinois'
                if sap_StateSlug.upper() == 'MD'.upper():
                    state_ = 'Maryland'
                if sap_StateSlug.upper() == 'MA'.upper():
                    state_ = 'Massachusetts'
                if sap_StateSlug.upper() == 'NJ'.upper():
                    state_ = 'New Jersey'
                if sap_StateSlug.upper() == 'NM'.upper():
                    state_ = 'New Mexico'
                if sap_StateSlug.upper() == 'NY'.upper():
                    state_ = 'New York'
                if sap_StateSlug.upper() == 'OH'.upper():
                    state_ = 'Ohio'
                if sap_StateSlug.upper() == 'PA'.upper():
                    state_ = 'Pennsylvania'
                if sap_StateSlug.upper() == 'WA'.upper():
                    state_ = 'Washington'
                        ...
                        if os.path.isfile(web_data_file):
                                ...
                                csv_a.writerow(
                                        ['ts', 'PremiseType', 'sku', 'BrandSlug', 'ChannelSlug', 'ProductName',
                                                'TermsOfServiceTyp', 'city_check', 'account_no', 'first_name', 'last_name',  'UtilitySlug', 'Commodity',
                                         'ServiceAddress1','ServiceAddress2', 'city', 'StateSlug', 'zip_code', 'email', 'emailmarketing'])
                                csv_a.writerow([ts_web, sap_PremiseType, sap_sku, sap_BrandSlug, sap_ChannelSlug, sap_ProductName,
                                                sap_TermsOfServiceType, city_check, str("'"+accountNo_1), first_name_generated,
                                                last_name_generated, sap_UtilitySlug,
                                               sap_Commodity, address_house_street_generated, address2, city,
                                                state_, generated_zipCode, email, emailmarketing])
                        time.sleep(1)
...
for row in input_file_epenet:
        for emailmarketing in email_mark:
                ...
                if epenet_State.upper() == 'CT'.upper():
                    state_ = 'Connecticut'
                if epenet_State.upper() == 'DE'.upper():
                    state_ = 'Delaware'
                if epenet_State.upper() == 'IL'.upper():
                    state_ = 'Illinois'
                if epenet_State.upper() == 'MD'.upper():
                    state_ = 'Maryland'
                if epenet_State.upper() == 'MA'.upper():
                    state_ = 'Massachusetts'
                if epenet_State.upper() == 'NJ'.upper():
                    state_ = 'New Jersey'
                if epenet_State.upper() == 'NM'.upper():
                    state_ = 'New Mexico'
                if epenet_State.upper() == 'NY'.upper():
                    state_ = 'New York'
                if epenet_State.upper() == 'OH'.upper():
                    state_ = 'Ohio'
                if epenet_State.upper() == 'PA'.upper():
                    state_ = 'Pennsylvania'
                if epenet_State.upper() == 'WA'.upper():
                    state_ = 'Washington'
                ...
                if epenet_UtilityAbbrev == 'wpp':
                    given_utiity = 'COLOHG'
                if epenet_UtilityAbbrev == 'COLOHG':
                    given_utiity = 'DUKEOHG'
                if epenet_UtilityAbbrev == 'DEOHG':
                    given_utiity = 'DEOHG'

                # Utility = epenet_UtilityAbbrev
                accountNo_1 =account_generator_accountNo(given_utiity)
                ...
                if epenet_Channel == 'inbound_telemarketing':
                        ...
                        if os.path.isfile(inbound_data_file):
                                ...
                                csv_a.writerow([ts_inb, epenet_PremiseType, epenet_sku, epenet_BrandSlug, epenet_Channel, epenet_BundleName, epenet_TermsOfServiceType, city_check,
                                                str("'"+str(accountNo_1)), first_name_generated, last_name_generated, epenet_UtilityAbbrev,
                                ...
                                csv_a.writerow(
                                        ['ts', 'PremiseType','sku', 'BrandSlug', 'ChannelSlug', 'ProductName',
                                                'TermsOfServiceTyp', 'city_check', 'account_no', 'first_name', 'last_name',  'UtilitySlug', 'Commodity',
                                         'ServiceAddress1','ServiceAddress2', 'city', 'StateSlug', 'zip_code', 'email', 'emailmarketing'])
                                csv_a.writerow([ts_inb,epenet_PremiseType, epenet_sku, epenet_BrandSlug, epenet_Channel,
                                                epenet_BundleName, epenet_TermsOfServiceType, city_check,
                                                str("'"+str(accountNo_1)), first_name_generated, last_name_generated, epenet_UtilityAbbrev,
                                               epenet_Commodity, address_house_street_generated, address2, city,
                                                state_, generated_zipCode, email, emailmarketing],)
...
read_file = pd.read_csv ('./b_files_for_testing_02/'+str(test_name)+'_web_data_file.csv')
