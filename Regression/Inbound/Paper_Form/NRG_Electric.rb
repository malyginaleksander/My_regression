...
ss=worksheet.cells(rows,"K").value.to_s
account_type=worksheet.cells(rows,"L").value
business_name=worksheet.cells(rows,"M").value
email=worksheet.cells(rows,"N").value
utility=worksheet.cells(rows,"O").value
account_number=worksheet.cells(rows,"S").value.to_s
service_ref=worksheet.cells(rows,"T").value.to_s
greenopt=worksheet.cells(rows,"U").value
partner=worksheet.cells(rows,"V").value
campaign=worksheet.cells(rows,"W").value
...
if 
browser.radio(:value => 'electric').visible?
browser.radio(:value => 'electric').set
end
...
#Order Information
browser.select_list(:name => 'offer_types_electric').select "Offer Code"
sleep 1
browser.text_field(:name => 'partner_code').set partner
sleep 1
browser.text_field(:name => 'campaign_code_offer').set campaign
sleep 1
if 
...
CSV.open('C:\\Scripts\\Inbound\\Paper_Form\\NRG_Electric_Conf_data.csv', 'ab') do |csv|
