...
for current_row in range(1,worksheet.nrows):
    ...
    service_ref = worksheet.row(current_row)[15].value
    greenopt = worksheet.row(current_row)[16].value
...
#Order Information
    ...
    if driver.find_element_by_name("promo_code").is_displayed():
        ...
        elem = driver.find_element_by_name("elec_promo_code").send_keys(promo)
    time.sleep(4)
...
# Grab Conformation Code
    ...
    confCode = elem.text
    print("Passed - EP Paper - Electric, " + confCode +' for ' + state +' - ' + given_utiity)
