...
#Zookeeper set user to RDI agent
...
driver.get("http://www.pt.energypluscompany.com/newadmin/operators.php")
elem = driver.find_element_by_id("prependedInput").send_keys("michael peters")
elem = driver.find_element_by_id("374").click()
time.sleep(2)
elem = driver.find_element_by_id("vendor_id")
for option in elem.find_elements_by_tag_name('option'):
    if option.text == ("RDIN : RDI Inbound"):
        option.click()
time.sleep(2)
...
## Run NRG_regression RDI Inbound Enrollment
workbook = xlrd.open_workbook("./RDI_Data.xlsx")
worksheet = workbook.sheet_by_name('RDI')
...
#Zookeeper set user to EPIB agent
driver = webdriver.Firefox() 
driver.get("http://www.pt.energypluscompany.com/newadmin/login.php")
...
elem = driver.find_element_by_id("vendor_id")
for option in elem.find_elements_by_tag_name('option'):
    if option.text == ("EPIB : Energy Plus Inbound"):
        option.click()
time.sleep(2)
elem = driver.find_element_by_id("save_button").click()
time.sleep(2)
...
print("Passed - RDI Inbound Enrollment")