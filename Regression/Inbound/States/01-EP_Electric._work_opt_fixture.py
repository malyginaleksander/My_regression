...
from Inbound.States.Inbound_Enrollments_4brands_test.InboundEnrollments_tests_Settings import URL
...
def grab_code(driver, payload):
    ...
    confcode = elem.text
    # time.sleep(2)
    print("Passed - EP Inbound Gas, Conformation =  " + confcode + ' for ' + payload.state + ' - ' + payload.utility)
    account_el = str(payload.accountNo_el)
    zip = str (payload.zip)
    f = open("./results.txt", 'a')
    now = datetime.now()
    time = now.strftime("%m_%d_%Y_%I_%M_%S_%p")
    f.write(confcode+"\n" )
    # f.write("Passed " + "|" + payload.first_name + "|" + sheet_name + "|" + payload.state + "|" + payload.utility + "|" + confcode + "|" + account_el +  "|" + payload.partner +
    #     "|" + payload.campaign + "|" + payload.promo + "|" + zip + "|" + payload.city + "|" + time + "\n" )
    #     "Passed " + "--" + payload.first_name + "--" + sheet_name + "--" + '  State ' + "--" + payload.state + "--" +
    #     ' utility = ' + "--" + payload.utility + "--" + confcode + "--" + payload.accountNo + "--" + payload.partner +
    #     "--" + "payload.compaign" + "--" + payload.promo + "--" + payload.zip + "--" + payload.city + "--" + time + "\n")
...
def test_state(driver, payload):
    now = datetime.now()
    time = now.strftime("%m_%d_%Y_%I_%M_%S_%p")
    # print(payload.first_name, sheet_name, ' - State = ', payload.state, ' - utility = ', payload.utility, time)
    ...
    try:
        ...
        print(str(ae))
        account = str(payload.accountNo_el)
        zip = str(payload.zip)
        f = open("./results.txt", 'a')
        f.write(
            "Passed " + "--" + payload.first_name + "--" + sheet_name + "--" + payload.state + "--" + payload.utility + "--"  + "--" + account + "--" + payload.partner +
            "--" + payload.campaign + "--" + payload.promo + "--" + zip + "--" + payload.city + "--" + time + "\n")
        raise ae
...
def EP_Electric_test(driver, payload):
    ...
    elem = driver.find_element_by_id(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.get_started_First_name_id).send_keys(payload.first_name)
    ...
    try:
        ...
        for option in elem.find_elements_by_tag_name('option'):
            if option.text == payload.first_plan:
                option.click()
    ...
    # time.sleep(2)
    elem = driver.find_element_by_class_name("uan").send_keys(str(payload.accountNo_el))
