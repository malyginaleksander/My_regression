...
service_reference=worksheet.cells(rows,"M").value.to_s
...
if
browser.select_list(:name => 'zone').exists?
browser.select_list(:name => 'zone').select("Westchester")
end
...
#Offer
sleep 4
#browser.link(:name => 'Pollution Free').click
...
CSV.open('C:\\Scripts\\Inbound\\States\\GME_State_Conf_data.csv', 'ab') do |csv|
