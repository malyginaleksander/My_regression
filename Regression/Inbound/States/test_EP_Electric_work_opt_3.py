...
from Inbound.States.Inbound_Enrollments_4brands_test.address_and_name_generator import last_name_generated, first_name_generated, \
    address_house_street_generated, random_zip, phone_area_code_generated, phone_prefix_generated, phone_last_generated, \
    city
...
#Todo - new york ???
    ...
    if payload.brand == 'NRG_regression':
        ...
        if payload.account_type_2 == "Gas":
            ...
            # time.sleep(1)
            # driver.find_element_by_id(payload.categorie_1).click()
            driver.find_element_by_xpath('//span[contains(text(),"2") and @class ="badge default-gray ng-binding"]/parent::h4/following-sibling::div[4]/p[2]/span/input').send_keys(payload.categorie_2)
    ...
    if payload.utility_type == 'Variable':
            try:
                Variable_buttons = driver.find_elements_by_xpath('//button[contains(text(), "Variable")]')
    ...
    if payload.account_type_2 == "Gas":
        ...
        # IF Electric choise on the TOP:
            try:
                elem = driver.find_element_by_xpath(
                    "//h4[@class='account-header']/span[contains(text(),'1')]/parent::h4/following-sibling::div[2]/div/p/select[@class='categories categories-dropdown form-control']")
                for option in elem.find_element_by_tag_name('option'):
                    ...
                    #new
                        ...
                        option.click()
                elem = driver.find_element_by_class_name("//h4[@class='account-header']/span[contains(text(),'1')]/parent::h4/following-sibling::div[2]/div/p[2]/select[@class='partners partners-dropdown form-control']")
                for option in elem.find_elements_by_tag_name('option'):
        ...
        # for gas choise
            try:
                ...
                for option in elem.find_elements_by_tag_name('option'):
                    if option.text == payload.partner_1:
                        option.click()
                elem = driver.find_element_by_xpath("//h4[@class='account-header']/span[contains(text(),'2')]/parent::h4/following-sibling::div[2]/div[2]/p/select[@class='campaigns campaigns-dropdown form-control']").click()
                elem = driver.find_element_by_xpath("//h4[@class='account-header']/span[contains(text(),'2')]/parent::h4/following-sibling::div[2]/div[2]/p/select[@class='campaigns campaigns-dropdown form-control']")
                for option in elem.find_elements_by_tag_name('option'):
                    if option.text == payload.campaign_2:
                            option.click()
                    if option.text == payload.campaign_1:
...
#todo do we need sleep?
    ...
    if payload.brand == 'GreenMT' or payload.brand == 'NRG_regression':
        try:
...
#Todo - do we need double twice choose
        ...
        if payload.brand == 'NRG_regression':
            if payload.utility_1 == 'Philadelphia Gas Works':
                driver.find_element_by_xpath("//span[contains(text(),'Service Point ID')]/following-sibling::input").send_keys(int(accountNo_1))
                driver.find_element_by_xpath("//span[contains(text(),'PGW Account Number')]/following-sibling::input[1]").send_keys(int(accountNo_1))
                driver.find_element_by_xpath("//span[contains(text(),'PGW Billing Account Number')]/following-sibling::input").send_keys(int(servicereference))
    ...
    elem = driver.find_element_by_name(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.billing_address_name).send_keys(address_house_street_generated)
    ...
    for option in elem.find_elements_by_tag_name(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.options_tag_name):
        if option.text == payload.state:
            option.click()
    elem = driver.find_element_by_name(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.billing_zip_name).send_keys(random_zip)
    elem = driver.find_element_by_name(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.billing_phone_area_code_name).send_keys(phone_area_code_generated)
    elem = driver.find_element_by_name(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.billing_phone_prefix_area_name).send_keys(phone_prefix_generated)
    elem = driver.find_element_by_name(Inbound.States.Inbound_Enrollments_4brands_test.Inbound_tags.billing_phone_last_digits_name).send_keys(phone_last_generated)
    elem = driver.find_element_by_class_name(
...
def grab_code(accountNo_1, payload):
    ...
    csv_w.writerow(["Passed", tester, payload.first_name, payload.brand, payload.state, payload.utility_1, confcode,
                    accountNo_1, payload.partner_1, payload.campaign_1, payload.promo_1, zip, payload.city, time])
    # f.write(confcode+"\n" )
