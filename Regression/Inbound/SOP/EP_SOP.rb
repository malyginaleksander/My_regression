...
zip=worksheet.cells(rows,"F").value
area_code=worksheet.cells(rows,"G").value
prefix=worksheet.cells(rows,"H").value.to_s
last=worksheet.cells(rows,"I").value.to_s
account_number=worksheet.cells(rows,"J").value.to_s
...
#Brand
browser.radio(:id => 'brandId_1').visible?
...
sleep 2
browser.link(:text, "Customer Info").click
sleep 5
...
CSV.open('C:\\Scripts\\Inbound\\SOP\\State_Conf_data.csv', 'ab') do |csv|
