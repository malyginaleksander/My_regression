...
worksheet = workbook.sheet_by_name('GME-pricing')
...
for current_row in range(1,worksheet.nrows):
    ...
    if driver.find_element_by_id("tbl_histpricing").is_displayed():
        print("Passed - GME Historic Pricing for " + state +' ' + commodity +' - ' + given_utiity)
    else:
       	print("Passed - GME Historic Pricing for " + state +' ' + commodity +' - ' + given_utiity)
...
for option in elem.find_elements_by_tag_name('option'):
    if option.text == ("Pennsylvania"):
        option.click()
time.sleep(3)
if driver.find_element_by_link_text("www.greenmountainenergy.com/PAHistoricalPricing").is_displayed():
    print ("Passed - GME Historic Pricing for Pennsylvania")
else:
    print ("Failed - GME Historic Pricing for Pennsylvania")
driver.close()