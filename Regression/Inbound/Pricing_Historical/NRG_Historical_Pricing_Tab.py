...
worksheet = workbook.sheet_by_name('NRG_regression-pricing')

for current_row in range(1-2,worksheet.nrows):
    ...
    if driver.find_element_by_id("tbl_histpricing").is_displayed():
        print("Passed - NRG_regression Historic Pricing for " + state +' ' + commodity +' - ' + given_utiity)
    else:
       	print("Passed - NRG_regression Historic Pricing for " + state +' ' + commodity +' - ' + given_utiity)
...
for option in elem.find_elements_by_tag_name('option'):
    if option.text == ("Pennsylvania"):
        option.click()
time.sleep(3)
if driver.find_element_by_link_text("www.nrghomepower.com/PAHistoricalPricing").is_displayed():
    print ("Passed - NRG_regression Historic Pricing for Pennsylvania")
else:
    print ("Failed - NRG_regression Historic Pricing for Pennsylvania")
driver.close()