...
base_file='./a_create_test_scenarios_for_campaign/inbox_files/all_tests_base_file.xlsx'
base_csv = './a_create_test_scenarios_for_campaign/outbox_files/all_tests_base_file.csv'
outbox_web_csv= './a_create_test_scenarios_for_campaign/outbox_files/' +test_name + '_web_test_scenarios.csv'
outbox_inbound_csv= './a_create_test_scenarios_for_campaign/outbox_files/' +test_name + '_inbound_test_scenarios.csv'

report_headers = ['ts', 'Brand','StateSlug',  'State', 'BrandSlug', 'PartnerCode', 'TermsOfServiceType', 'PremiseType',
                     'Commodity', 'ChannelSlug', 'SKU', 'Bonus', 'Ongoing_Earn',
                     'promo_compaign_code', 'PromoCode', 'UtilitySlug', 'Offer', 'ECF_NoECF', 'ProductName',
                     'Bundle_Description', 'ProductSlug', 'System',  'utility_inb',
...
if delete_files.lower() == "y".lower():
    for file in os.listdir('./a_create_test_scenarios_for_campaign/outbox_files/'):
        os.remove('./a_create_test_scenarios_for_campaign/outbox_files/' + file)
time_= datetime.now()
    ...
    if ChannelSlug == 'inbound_telemarketing':
        if make_inbound == 1:
            ...
            if os.path.isfile(outbox_inbound_csv):
                ...
                csv_a.writerow(
                    [ts_inb_name, 'NRG', StateSlug, state_full_name, BrandSlug, Partner, TermsOfServiceType, PremiseType,
        ...
        if make_web == 1:
            ...
            if os.path.isfile(outbox_web_csv):
                ...
                csv_a.writerow(
                    [ts_web_name, 'NRG', StateSlug,  state_full_name, BrandSlug,  Partner, TermsOfServiceType, PremiseType,
                     Product, ChannelSlug, SKU, Bonus, Ongoing_Earn,
                     Campaign_Code, promo_code_fixed, UtilitySlug, Offer, ECF_NoECF, ProductName,
                     Bundle_Description, ProductSlug, System,  utility_inb,
                     first_name_generated, last_name_generated, address_house_street_generated,
...
if make_inbound==1:
    ...
    read_file.to_excel ('./a_create_test_scenarios_for_campaign/outbox_files/'+str(test_name)+'_inbound_data_file.xlsx', index = None, header=True)
    read_file.to_excel ('./b_files_for_testing_02/web_data_file.xlsx', index = None, header=True)
else:
    pass

if make_web ==1:
    read_file = pd.read_csv (outbox_web_csv)
    read_file.to_excel ('./a_create_test_scenarios_for_campaign/outbox_files/'+str(test_name)+'_web_data_file.xlsx', index = None, header=True)
else:
    pass
