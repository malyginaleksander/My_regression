...
workbook = xlrd.open_workbook("./Save_Enrollment_Data.xlsx")
worksheet = workbook.sheet_by_name(NRG)
...
## Dispo
    ...
    time.sleep(3)
    elem = driver.find_element_by_id("dispo-list")
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == ("509 : Test Call"):
            option.click()
            time.sleep (2)
...
## Load Saved Call
    elem = driver.find_element_by_link_text("Load Saved Enrollment").click()
    time.sleep(2)
    elem = driver.find_element_by_id("caller-first-name").send_keys(fname)
    elem = driver.find_element_by_id("account-first-name").send_keys(fname)
    time.sleep(2)
    elem = driver.find_element_by_xpath("/html/body/div[2]/div/div[1]/div[1]/div[1]/form/button").click() #submit
    time.sleep(2)
    elem = driver.find_element_by_xpath("/html/body/div[2]/div/div[1]/div[1]/div[3]/table/tbody/tr[1]/td[1]").click()
    time.sleep(4)
    elem = driver.find_element_by_xpath("/html/body/div[2]/div/div[1]/div[1]/div[4]/div/div/div[3]/button[2]").click()
    time.sleep(8)

    try:
        if driver.find_element_by_class_name("disabled").is_displayed():
            print("Test Case Passed - Proceed to Disclosures is disabled")
    except:
        print("Test Case Failed - Proceed to Disclosures is enabled")        

## Billing Info:
    elem = driver.find_element_by_link_text("Billing Info").click()
...
## Grab Conformation Code
    ...
    confcode = elem.text
    print("Passed - NRG_regression Inbound Load Save Call, Conformation =  " + confcode + ' for ' + state + ' - ' + given_utiity)
