...
def test_file_query():
    ...
    for file in input_files_list:
        ...
        if 'energyplus' in file:
            ...
            #
            # query_text = (
            #     """select item.sap_enrollment_confirmation, ord.order_id, ord.marketer_id, ord.order_status, ord.created, ord.enrollment_number, ord.source_filename, item.uan, item.sku, item.uan_valid,
            #         item.created, item.date_of_Sale, item.external_id, item.representative_id, item.partner_code
            #         from nerf_order ord
            #         join nerf_fileenrollment file
            #         on ord.source_filename = file.file_name
            #         join nerf_orderitem item
            #         on ord.order_id = item.order_id where ord.created > '2020-05-18% 00:00'""").format(file)
...
# on ord.source_filename = file.file_name
# join nerf_orderitem item
# on ord.order_id = item.order_id where ord.created> '2020-05-12%'""")#
#             query_text = (
#                 """select item.sap_enrollment_confirmation, ord.order_id, ord.marketer_id, ord.order_status, ord.created, ord.enrollment_number, ord.source_filename, item.uan, item.sku, item.uan_valid,
#                     item.created, item.date_of_Sale, item.external_id, item.representative_id, item.partner_code
#                     from nerf_order ord
...
#                     on ord.source_filename = file.file_name
#                     join nerf_orderitem item
#                     on ord.order_id = item.order_id where file_name= 'all05162020-213007.txt'""")
#             query_text = (
#                 """select item.sap_enrollment_confirmation, ord.order_id, ord.marketer_id, ord.order_status, ord.created, ord.enrollment_number, ord.source_filename, item.uan, item.sku, item.uan_valid,
# item.created, item.date_of_Sale, item.external_id, item.representative_id, item.partner_code
# from nerf_order ord
...
# on ord.source_filename = file.file_name
# join nerf_orderitem item
# on ord.order_id = item.order_id where file_name= 'enrollments_j111485_05142020-101935.txt'""")
