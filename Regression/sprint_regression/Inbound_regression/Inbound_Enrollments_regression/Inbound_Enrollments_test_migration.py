...
from Regression.Inbound.States.Inbound_Enrollments_4brands_test.zip_city_generator import find_zip_city
...
def grab_code(accountNo_1, payload):
    ...
    report_csv = "./outbox_folder/Inbound_Enrollments_tests_results.csv" + str(current_time) + ".csv"
    if os.path.isfile(report_csv):
        ...
        csv_a.writerow(
                [payload.test_name,  payload.brand, payload.account_type_1, payload.account_type_2,
                 ...
                 payload.categorie_1, str("'" + str(payload.zip)),  payload.state, payload.city,  payload.ServiceAddress1, payload.email,
                 payload.emailmarketing, str("'" + str(accountNo_1)), payload.sku, confcode, time ])
    else:
        f = open(report_csv, 'a', newline='')
        csv_a = csv.writer(f)
        csv_a.writerow(
            ['test_name',  'brand', 'account_type_1', 'account_type_2',
                 ...
                 'categorie_1', 'zip',  'state', 'city',  'ServiceAddress1', 'email',
                 'emailmarketing', 'accountNo_1','sku','confcode', 'time'])
...
def Inbound_start_test(test_setup, payload):
    ...
    generated_zipCode, generated_city = find_zip_city(payload.last_name, payload.state)
...
def fill_billing_info_page(address_house_street_generated,  payload, phone_area_code_generated,
    ...
    for option in elem.find_elements_by_tag_name(options_tag_name):
        if option.text == payload.state:
            option.click()
    elem = driver.find_element_by_name(billing_zip_name).send_keys(str(payload.zip))
...
def fill_get_started_page(first_name_generated_, last_name_generated_, payload):
    ...
    driver.find_element_by_id(get_started_First_name_id).send_keys(name)
    ...
    if payload.account_type_1 == 'Electric':
        driver.find_element_by_xpath('//option[@value="'+payload.utility_1+'"]').click()

        # elem = driver.find_element_by_name(get_started_EL_utility_1_class_name)
        # for option in elem.find_elements_by_tag_name(options_tag_name):
        #     if option.value.upper() == payload.utility_1:
        #         option.click()
    # if payload.account_type_1 == 'Gas':
    #     # try:
