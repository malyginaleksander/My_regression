...
def fill_billing_info_page(driver, payload, firstname, lastname, address, zipcode_, city, accountNo, email,  phonenumber  ):
    ...
    driver.find_element_by_name(billing_phone_area_code_name).send_keys(phonenumber[:3])
    driver.find_element_by_name(billing_phone_prefix_area_name).send_keys(phonenumber[3:6])
    driver.find_element_by_name(billing_phone_last_digits_name).send_keys(phonenumber[6:])
