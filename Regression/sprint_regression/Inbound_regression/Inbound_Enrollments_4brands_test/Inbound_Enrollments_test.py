...
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.InboundEnrollments_tests_Settings import *
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_BrandPage import BrandPage
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_accountNO_generator import *
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_generator_names_and_address import *
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_generator_zip_city import *
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_loginPage import *
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_pages_methods import *
from  Regression.sprint_regression.Inbound_regression.Inbound_Enrollments_4brands_test.Inbound_tags import *

sheet_name = data_sheet_name
workbook = xlrd.open_workbook('C:\\Users\\AMALYGIN\\Downloads\Regression-master (5)\\Regression-master\\Regression\\sprint_regression\\Inbound_regression\\Inbound_Enrollments_4brands_test\\'
                              'InboundTest_Data.xlsx')
...
def grab_code(accountNo_1, payload,first_name_generated):
    now = datetime.now()
    date= now.strftime("%m_%d_%Y")
    ServiceAddress1 = str(payload.first_name) + " Street"
    wait_grab_code_page(driver)
    elem = driver.find_element_by_id("confcode")
    confcode = elem.text
    report_name = "./outbox_folder/Inbound_Enrollments_tests_results_{}.csv".format(date)
    if os.path.isfile(report_name):
        f = open(report_name, 'a', newline='')
        csv_a = csv.writer(f)
        csv_a.writerow(
            [payload.test_name, payload.brand, payload.account_type_1, payload.account_type_2,
         first_name_generated, payload.last_name,  payload.utility_1, payload.type,
         payload.categorie_1, str("'" + str(payload.zip)), payload.state, payload.city,  ServiceAddress1, str("'" + str(accountNo_1)), confcode, 'passed'])
    else:
        f = open(report_name, 'a', newline='')
        csv_a = csv.writer(f)
        csv_a.writerow(
            ['test_name', 'brand', 'account_type_1', 'account_type_2',
         'first_name', 'last_name',  'utility_1', 'type',
         'categorie_1', 'zip', 'state', 'city',  'ServiceAddress1', 'accountNo_1', 'confcode', 'status'])
        csv_a.writerow(
            [payload.test_name, payload.brand, payload.account_type_1, payload.account_type_2,
         first_name_generated, payload.last_name,  payload.utility_1, payload.type,
         payload.categorie_1, str("'" + str(payload.zip)), payload.state, payload.city,  ServiceAddress1, str("'" + str(accountNo_1)), confcode, 'passed'])
...
def Inbound_start_test(test_setup, payload):
    ...
    # account_no = account_generator_accountNo_1(payload)
    ...
    fill_get_started_page(first_name_generated,  payload)
    ...
    elem = driver.find_element_by_name(customer_firs_name_name).send_keys(first_name_generated)
    ...
    grab_code(accountNo_1, payload, first_name_generated)
...
def fill_get_started_page(first_name_generated,  payload):
    driver.find_element_by_id(get_started_First_name_id).send_keys(first_name_generated)
