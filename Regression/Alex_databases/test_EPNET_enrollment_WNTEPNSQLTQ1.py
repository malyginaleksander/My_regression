...
folder_location = './inbox_files'

pattern_email = r'[\w\.-]+@[\w\.-]+\.com'
pattern_date = r'\d{4}-\d{2}-\d{2}'
pattern_header = r'[%Status%]'
...
def read_inbox_files(pattern_email, pattern_date):
    ...
    for file in filesList:
            ...
            if os.path.getsize(FileNameOpen) > 0:
                for i in f:
                    header = re.search(pattern_header, i, re.IGNORECASE)
                    ...
                    if email == None or date == None:
                        if header: #to don't analyze header for mistakes
                            pass
                        else:
                            if email == None:
...
def test_verifivation():
    ...
    if count_files_in_folder > 0:
        ...
        if len(listofemails) > 0 and len(listofdates) >0:
            ...
            csv_from_txt_path = './b_files_for_testing_02/Report_{}_.csv'.format(current_time)
            failed_csv_from_txt_path = './b_files_for_testing_02/Failed_report_{}.csv'.format(current_time)
