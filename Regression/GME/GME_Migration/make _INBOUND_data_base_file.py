...
sku_file_xlsx = "./Inbox_files/inbound_skus.xlsx"
sku_file_csv = "./Inbox_files/inbound_skus.csv"
test_data_file_csv = "./Inbox_files/test_scenarios_INBOUND_file.csv"
test_data_file_xlsx = "./Inbox_files/test_scenarios_INBOUND_file.xlsx"
...
elif emailmarketing_test==1:
    email_mark = ['no_email_mark', 'emailmarketing', 'bouncedmai']
    # email_mark = ['emailmarketing', 'bouncedmail']
done_utility_list = []
...
for row in sku_scv_dict:
    for emailmarketing in email_mark:
        ...
        if str(UtilitySlug+emailmarketing) in done_utility_list:
            pass
        else:
            done_utility_list.append(str(UtilitySlug+emailmarketing))
            ...
            bouncedmail_list = ['thiswillbounce@thisbounces.com',
                                'bouncedmail@eccbounce.com',
                                'bouncermail@eccbounced.com',
                                'bouncy@mcbounceface.com']

            if emailmarketing== 'bouncedmail':
                email =random.choice(bouncedmail_list)
            else:
                email = 'aleksandr.malygin@nrg.com'
            if StateSlug.upper() == 'IL':
                state_ = 'Illinois'

            elif StateSlug.upper()  == 'PA':
                state_ = 'Pennsylvania'


            elif StateSlug.upper()  == 'MA':
                state_ = 'Massachusetts'

            elif StateSlug.upper()  == 'NJ':
                state_ = 'New Jersey'

            elif StateSlug.upper()  == 'MD':
                state_ = 'Maryland'

            elif StateSlug.upper()  == 'OH':
                state_ = 'Ohio'

            elif StateSlug.upper()  == 'NY':
                state_ = 'New York'
