...
workbook=excel.Workbooks.Open("C:\\Scripts\\EP_Web\\Enrollments\\PA\\PA_Data.xlsx") #Edit this to point to your location.
...
busnamedet=worksheet.cells(rows,"T").value
pfname=worksheet.cells(rows,"U").value
plname=worksheet.cells(rows,"V").value
price=worksheet.cells(rows,"W").value
con1=worksheet.cells(rows,"Y").value
con2=worksheet.cells(rows,"Z").value
city=worksheet.cells(rows,"AA").value
...
browser.div(:id => 'AccountMainElectric').select_list(:class => 'average_month_usage').select ('Please Choose...')
browser.div(:id => 'AccountMainElectric').select_list(:class => 'average_month_usage').fire_event 'onClick'
browser.div(:id => 'AccountMainElectric').select_list(:class => 'average_month_usage').select ('Less than 10,000 kWh')
browser.div(:id => 'AccountMainElectric').select_list(:class => 'average_month_usage').fire_event 'onClick'
end
...
#Gas
if
browser.div(:id => 'AccountMainGas').exists?
...
browser.div(:id => 'AccountMainGas',).select_list(:class => 'resbus').fire_event 'onClick'
if
browser.select_list(:class => 'gastypesel').exists?
...
if
browser.div(:id => 'AccountMainGas',).text_field(:class => 'usage_units').exists?
...
CSV.open('C:\\Scripts\\EP_Web\\Enrollments\\PA\\PA_Conf_data.csv', 'ab') do |csv|
csv << [browser.span(:id => 'confirmationCode').text, price=worksheet.cells(rows,"W").value, con1=worksheet.cells(rows,"Y").value,
con2=worksheet.cells(rows,"Z").value, con3=worksheet.cells(rows,"AA").value]
end
