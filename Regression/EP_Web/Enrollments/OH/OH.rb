...
workbook=excel.Workbooks.Open("C:\\Scripts\\EP_Web\\Enrollments\\OH\\OH_Data.xlsx") #Edit this to point to your location.
...
accountNo=worksheet.cells(rows,"Q").value.to_s #value.to_s removes error for limit_to_maxlength when cells have large numbers
busnamedet=worksheet.cells(rows,"R").value
pfname=worksheet.cells(rows,"S").value
plname=worksheet.cells(rows,"T").value
price=worksheet.cells(rows,"U").value
con1=worksheet.cells(rows,"W").value
con2=worksheet.cells(rows,"X").value
city=worksheet.cells(rows,"Y").value
...
CSV.open('C:\\Scripts\\EP_Web\\Enrollments\\OH\\OH_Conf_data.csv', 'ab') do |csv|
csv << [browser.span(:id => 'confirmationCode').text, price=worksheet.cells(rows,"U").value, con1=worksheet.cells(rows,"W").value,
con2=worksheet.cells(rows,"X").value, con3=worksheet.cells(rows,"Y").value]
end
