...
worksheet = workbook.sheet_by_name('EP Choice Web')
...
for current_row in range(1,worksheet.nrows):
    ...
    Service_City = worksheet.row(current_row)[9].value
    zipcode = worksheet.row(current_row)[10].value
    phone = worksheet.row(current_row)[11].value
    elect_gas_radio = worksheet.row(current_row)[12].value
    LocalUtility = worksheet.row(current_row)[13].value
    account_type = worksheet.row(current_row)[14].value
    greenopt_check = worksheet.row(current_row)[15].value
    gastypesel = worksheet.row(current_row)[16].value
    accountNo = worksheet.row(current_row)[19].value
    busnamedet = worksheet.row(current_row)[22].value
    pfname = worksheet.row(current_row)[24].value
    plname = worksheet.row(current_row)[25].value
    
    driver = webdriver.Firefox()
    
    driver.get("http://www.energypluscompany.com/sywr/choice/")
...
# Personal Information
    ...
    elem = driver.find_element_by_id("Service_City").send_keys(Service_City)
...
#Grab Conformation Code
    ...
    confcode = elem.text
    print("Passed - EP Web Production Choice page, Confirmation =  " +confcode + ' for - ' +state + ' - ' +LocalUtility)
