...
worksheet = workbook.sheet_by_name('Paper EP Gas')
...
for current_row in range(1,worksheet.nrows):
    ...
    business_name = worksheet.row(current_row)[11].value
    email = worksheet.row(current_row)[12].value
    given_utiity = worksheet.row(current_row)[13].value
    account_number = worksheet.row(current_row)[16].value
...
#Order Information
    ...
    elem = driver.find_element_by_name("campaign_code").send_keys(campaign)
    elem = driver.find_element_by_name("gas_promo_code").send_keys(promo)
    time.sleep(4)
...
# Grab Conformation Code
    ...
    confCode = elem.text
    print("Passed - EP Paper - Gas Production, " + confCode +' for ' + state +' - ' + given_utiity)
