...
Payload = namedtuple('payload', ['tc', 'brand', 'first_name', 'last_name', 'address', 'city', 'state', 'zipcode', 'area_code', 
    'prefix', 'last', 'account_type', 'email', 'utility', 'account_number', 'partner', 'campaign', 'promo'])
...
def test_state(driver, payload):
    ...
    for option in elem.find_elements_by_tag_name('option'):
        if option.text == payload.utility:
            option.click()
    elem = driver.find_element_by_name("gas_account_number").send_keys(payload.account_number)
    ...
    for option in elem.find_elements_by_tag_name('option'):
    	if option.text == ("Offer Code"):
    		option.click()
    elem = driver.find_element_by_name("partner_code_gas").send_keys(payload.partner)
    elem = driver.find_element_by_name("campaign_code_gas").send_keys(payload.campaign)
    if driver.find_element_by_name("promo_code_gas").is_displayed():
        elem = driver.find_element_by_name("promo_code_gas").send_keys(payload.promo)
    time.sleep(4)
