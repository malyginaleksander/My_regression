...
Payload = namedtuple('payload', ['tc', 'brand', 'first_name', 'last_name', 'address', 'city', 'state', 'zipcode', 'area_code', 
    'prefix', 'last', 'account_type', 'business_name', 'email', 'utility', 'account_number', 'service_ref', 'greenopt',
    'partner', 'campaign', 'promo'])

local_path = './ep_web_production/prod_data.xlsx'
full_path = os.path.abspath(local_path)
workbook = xlrd.open_workbook(full_path)
worksheet = workbook.sheet_by_name('Paper EP Electric')
...
paper_data = [
    ...
    greenopt = worksheet.row(current_row)[17].value,
    partner = worksheet.row(current_row)[18].value,
    campaign = worksheet.row(current_row)[19].value,
    promo = worksheet.row(current_row)[20].value,
    )
...
def driver(request):
    ...
    if os.environ.get('USE_PHANTOM'):
        ...
        _driver.get("http://www.pt.energypluscompany.com/myinbound/login.php")
        elem = _driver.find_element_by_name("email").send_keys("mpeters@energypluscompany.com")
        elem = _driver.find_element_by_name("password").send_keys("energy")
...
def test_state(driver,payload):
    print()
    print("Inside test_state method")
    #print("Passed - EP Paper - Electric, " +' for ' +' Utility= '+payload.Utility+'- account_type = ' +payload.Account_Type+'- greenopt ='+payload.green)
    #Capture Failed Testcases 
    try:
