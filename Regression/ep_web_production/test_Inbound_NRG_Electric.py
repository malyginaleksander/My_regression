...
def test_state(driver, payload):
    print(payload.tc, 'NRG_regression Inbound Production Electric Enrollment - State = ', payload.state, ' - utility = ', payload.utility)
    ...
    ##call process
    energyPlusFill.start_call_NRG_electric()

    ### Get Started:
    energyPlusFill.get_started_in_call(payload)

    # Electric Account steps
    energyPlusFill.nrg_electric_acct_steps(payload)

    ## Offer:
    energyPlusFill.Inbound_Offers(payload)
    ...
    #disclosure
    energyPlusFill.nrg_elec_disclosure_activity(payload)
